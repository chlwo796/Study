...
public class Pager {
	...
	public static String makeTag(HttpServletRequest request , int pageSize , int total) {
		String Tag = "" ; 
		String contextPath = request.getContextPath();  // http://localhost:9000/myhome
		...
		int pageGroupSize = 5; 	//한 그룹당 보여질 페이지 수 
		
		//1 ~5 
		//6 ~ 10
		//11 ~ 15
		int pageGroupStart; //1, 6,11,16,... 그룹의 시작
		int pageGroupEnd;   //5,10,15,20.... 그룹의 끝
		...
		try {
			...
			//http://localhost:9000/myhome/board/list?pg=1
...
//			setTotalCnt(Integer.parseInt(totCnt)) ; 
			...
			cpage = Integer.parseInt(page) ; //현재페이지값 

			pageTotal = (int)Math.ceil((total - 1) / pageSize); 
			//total - 전체 데이터 건수 
			//전체 페이지 개수 구하기 
            
			//페이지 그룹값 설정하기 만일 현재 12페이지라면 그룹은 3가 되어야 한다 

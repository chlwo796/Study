...
SELECT DATE_FORMAT(UI_CREDAT, '%Y-%m-%d') UI_CREDAT, COUNT(*) FROM USER_INFO GROUP BY DATE_FORMAT(UI_CREDAT, '%Y-%m-%d') ORDER BY UI_CREDAT ASC;

SELECT * FROM USER_INFO WHERE UI_ACTIVE = 1;

UPDATE USER_INFO SET UI_FILEPATH = '/resources/upload/adminPhoto.jpg' WHERE UI_NUM = 160;

COMMIT;

DELETE FROM USER_INFO WHERE UI_NUM = 114;
DELETE FROM KAKAO_USER_INFO WHERE KUI_ID = 2830205145;

SELECT * FROM BABSANG_INFO WHERE BI_CLOSED = 0;
SELECT * FROM BABSANG_INFO WHERE BI_CLOSED = 1;
SELECT * FROM BABSANG_INFO WHERE BI_USERCNT > 1;
SELECT DATE_FORMAT(UI_CREDAT, '%Y-%m-%d') UI_CREDAT, COUNT(*) UI_USERCNT FROM USER_INFO 
GROUP BY DATE_FORMAT(UI_CREDAT, '%Y-%m-%d') ORDER BY UI_CREDAT ASC;
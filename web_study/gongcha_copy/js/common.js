				...
				success: function(html){
					...
					if($.trim(html)!=""){
						...
						$("#store_list ul li a").unbind("click").bind("click", function(){
							...
							Map.SelectStore($(this).parent().index());
						});
...
Map = {
				...
				success: function(data){
					...
					if(data){
						...
						for (var i = 0; i < positions.length; i ++) {
							...
							Map.Maker(map, positions[i].latlng, content_no, i);
						}
					}
				},
				error: function(xhr){
	...
	Maker : function(map, position, content_no, i){
		...
		var markerOnImage  = Map.CreateMarkerImage("on");
		var markerOffImage = Map.CreateMarkerImage("off");
...
Application = {
	Init : function(){
		...
		$("#frmWrite").submit(function(){
			...
			var cnt1=0;
			for(i=0; i<$("input[name='etc13']").length; i++){
				if($("input[name='etc13']").eq(i).prop("checked")) cnt1++;
			}
			if(cnt1==0) {
				alert("[창업 희망시기]을 선택해주세요.");
				return false;
			}

			var cnt2=0;
			for(i=0; i<$("input[name='etc14[]']").length; i++){
				if($("input[name='etc14[]']").eq(i).prop("checked")) cnt2++;
			}
			if(cnt2==0) {
				alert("[희망상권]을 선택해주세요.");
				return false;
			...
			var cnt3=0;
			for(i=0; i<$("input[name='etc15']").length; i++){
				if($("input[name='etc15']").eq(i).prop("checked")) cnt3++;
			}
			if(cnt3==0) {
				alert("[투자비용]을 선택해주세요.");
				return false;
